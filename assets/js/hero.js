(function () {
  ...
  var FRAG = [
    ...
    ' c=mix(c,vec3(0.500,0.360,0.150),smoothstep(0.54,0.68,t));',
    ' c=mix(c,vec3(0.700,0.470,0.180),smoothstep(0.66,0.78,t));',
    ' c=mix(c,vec3(0.780,0.610,0.290),smoothstep(0.78,0.89,t));',
    ' c=mix(c,vec3(0.800,0.730,0.580),smoothstep(0.90,1.00,t));',
    ' return c;}',
    'void main(){',
    ...
    ' float halo=exp(-d*2.4)*0.34+exp(-d*5.5)*0.45;',
    ' col+=vec3(1.0,0.84,0.48)*halo;',
    ' col+=vec3(1.0,0.93,0.66)*exp(-d*10.0)*0.30;',
    ' float disc=smoothstep(0.086,0.072,d);',
    ' col=mix(col,vec3(0.99,0.93,0.76),disc*0.85);',
    ' float ang=atan(sd.y,sd.x);',
    ' float rays=(0.5+0.5*sin(ang*30.0+u_t*0.18))*(0.5+0.5*sin(ang*13.0-u_t*0.10));',
    ' rays*=smoothstep(0.64,0.05,d);',
    ' col+=vec3(1.0,0.88,0.60)*rays*0.04;',
    ' col*=mix(0.62,1.0,smoothstep(0.0,0.42,uv.y));',
    ' col*=mix(0.74,1.0,smoothstep(1.25,0.30,length((uv-0.5)*vec2(asp,1.0))));',
    ' col*=0.82;',
